...
class AdventureService:
    ...
    def evaluate_consecutivedays_challenges(self, week_number, year_number):
        ...
        for challenge in challenges:
            ...
            self.notion_service.persist_adventure(adventure=challenge, characters=[who], prefix='challenges')
        return challenges
    ...
    def evaluate_weekhabits_challenges(self, week_number, year_number):
        ...
        for challenge in challenges:
            ...
            challenge['encounter_log'] = self.encounter_log
            challenge['dlylog'] = dlylog_array
            self.notion_service.persist_adventure(adventure=challenge, characters=gods_winner, prefix='challenges')
        return challenges
    ...
    def evaluate_expired_challenges(self,week_number, year_number):
        ...
        for challenge in due_challenges:
            ...
            challenge['status'] = 'lost'
            challenge['encounter_log'] = self.encounter_log
            upd_adventure, upd_character = self.notion_service.persist_adventure(adventure=challenge, characters=pool_whos, prefix='challenges')
        ...
        for challengew in won_challenges:
            ...
            upd_adventure, upd_character = self.notion_service.persist_adventure(adventure=challengew, characters=pool_whos, prefix='challenges')
            upd_challenges.append({ 'adventure_id':upd_adventure['id']
                                , 'who_id':upd_character['id']
    ...
    def evaluate_challenges_due_soon(self, lookforward):
        ...
        for challenge in reorder_challenge:
            ...
            if not cached_notification:
                ...
                self.redis_service.set_with_expiry(cached_key, { "content": heading + challenge['name']
                                                                        , "due_date": due_date
                                                                        , "priority": priority
                ...
                if challenge['due'] >= today_str:
                    ...
                    self.redis_service.set_with_expiry(self.redis_service.get_cache_key_nomerge('notification', 'deadline', challenge['id'])
                                                                            ...
                                                                            , "section_id": None, "labels": None}, expirity_hours)
        return {'challenges': reorder_challenge}
            ...
            for challenge in challenges_array:
                ...
                if not cached_notification:
                    self.redis_service.set_with_expiry(cached_key, { "content": heading + challenge['name']
                                                                            ...
                                                                            , "priority": 1
                                                                            , "description": f'last time checked on __{today_str}__'
                                                                            , "section_id": None, "labels": [dbid['desc']]}, self.expiry_hours)
        ...
        if not self.redis_service.exists(self.redis_service.get_cache_key(key_str,'prsnl')):
            ...
            for key in keys:                
                ...
                if not self.redis_service.exists(cached_key):
                    ...
                    self.redis_service.set_with_expiry(cached_key, { "content": content
    ...
    def execute_adventure(self, adventure_id):
        ...
        if status in ('created', 'accepted', 'on going'):
            ...
            self.notion_service.persist_adventure(adventure=adventure, characters=enemies, prefix='adventure')
        return {
    ...
    def execute_underworld(self, limit=0):
        ...
        for deaadventure in sample_adventures:
            ...
            self.notion_service.persist_adventure(adventure=deaadventure, characters=[who,enemy] , prefix='deadventures')
    ...
    def apply_punishment(self):
        ...
        for adventure in all_adventures:
            ...
            adventure['encounter_log'] = self.encounter_log
            self.notion_service.persist_habit(habit)
            self.notion_service.persist_adventure(adventure=adventure, characters=[character], prefix='punishment')
